...
async def add_write_off(parsed_text: list[tuple[int, str, str, str, float, str, datetime]]) -> None:
    """
    Добавляет новые записи в таблицу списаний.

    :param parsed_text: str (форматированный текст списания)
    :return: None
    """
    type_id = await get_write_off_type_id(parsed_text[0][1])
...
async def init_units_of_measurement(session: AsyncSession) -> None:
    ...
    for unit in units:
        exists = await session.scalar(
            select(m.UnitsOfMeasurementOrm).where(m.UnitsOfMeasurementOrm.name == unit["name"])
        )
        if not exists:
...
async def get_unit_of_measurement_id(unit_name: str) -> int:
    ...
    async with async_session() as session:
        unit_id = await session.scalar(
            select(m.UnitsOfMeasurementOrm.id).where(m.UnitsOfMeasurementOrm.name == unit_name)
        )
    return unit_id